...
class IngerirDespesasPorMesUseCase:
    def __init__(self, portal_port, core_port):
        self.portal_port = portal_port
        self.core_port = core_port

    def executar(self, ano: int, mes: int, gestao: str, fase: int = 3, max_paginas: int = 10) -> ResultadoIngestaoMensal:
        ...
        resultado = ResultadoIngestaoMensal(ano=ano, mes=mes, dias_processados=0, dias_sem_dados=0, dias_com_erro=0, total_criadas=0, total_erros=0)
        ...
        logger.info(
            f"{mes:02d}/{ano} concluído: {resultado.dias_processados} dias com dados, "
